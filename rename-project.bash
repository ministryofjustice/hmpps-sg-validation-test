...
find . $EXCLUDES -prune -o -type f -exec sed -i \
  -e "s/hmpps-sg-validation-test/$PROJECT_NAME/g" \
  -e "s/sg-validation-test/$PROJECT_NAME_WITHOUT_HMPPS/g" \
  -e "s/HMPPS Sg Validation Test/$PROJECT_DESCRIPTION/g" {} \;

echo "Performing directory renames"

# move helm stuff to new name
mv "helm_deploy/hmpps-sg-validation-test" "helm_deploy/$PROJECT_NAME"
